...
class Sendler_msg:
            ...
            try:
                ...
                try:
                    ...
                    if str(es) == 'Chat not found':
                        logger_msg(f'Бот не имеет права писать админу, напишите /start боту')
                    else:
                        logger_msg(f'Ошибка при отправке сообщение админу текст: "{text}" {es}')
    ...
    async def sendler_to_admin(message: Message, text, keyb):
        ...
        for admin in ADMIN:
            try:
                ...
                try:
                    res_send = await message.bot.send_message(int(admin), text)
                except Exception as es:
                    if str(es) == 'Chat not found':
                        logger_msg(f'Бот не имеет права писать админу, напишите /start боту')
                    else:
        ...
        for admin in ADMIN:
            try:
                ...
                try:
                    ...
                    if str(es) == 'Chat not found':
                        logger_msg(f'Бот не имеет права писать админу {admin}, напишите /start боту')
                    else:
                        logger_msg(f'Ошибка при отправке сообщение админу текст {admin},: "{text}" {es}')
    ...
    async def send_msg_call(call: types.CallbackQuery, text_msg, keyb):
        try:
            ...
            # logger_msg(f'Ошибка редактирования поста: {es}')
            try:
                ...
                try:
                    ...
                    logger_msg(f'Произошла ошибка при отправке поста текст: "{text_msg}" ошибка: "{es}"')
                    return False
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger_msg(f'Произошла ошибка при отправке поста текст2: "{text_msg}" ошибка: {es}"')
                        return False
    ...
    async def send_call_message(call: types.CallbackQuery, text_msg, keyb):
        if call.message.photo != []:
            ...
            try:
                ...
                logger_msg(f'Произошла ошибка при отправке поста текст: "{text_msg}" send_call_message"')
                return False
        else:
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger_msg(f'Произошла ошибка при отправке поста текст: "{text_msg}" send_call_message2 {es}"')
                        return False
    ...
    async def reply_user(message: Message, text_msg):
        ...
        try:
            ...
            logger_msg(f'Произошла ошибка при reply_user текст: "{text_msg}" ошибка: "{es}"')
            return False
    ...
    async def sendler_photo_call(self, call, photo, text, keyb):
        try:
            ...
            try:
                ...
                logger_msg(f'Ошибка при отправке сообщения call с фото {es}')
                return False
    ...
    async def new_sendler_photo_call(self, call, photo, text, keyb):
        ...
        try:
            ...
            logger_msg(f'Ошибка при отправке сообщения new_sendler_photo_call с фото {es}')
            return False
    ...
    async def sendler_photo_message(self, message, photo, text, keyb):
        try:
            ...
            try:
                ...
                logger_msg(f'Ошибка при отправке сообщение msg с фото {es}')
                return False
    ...
    async def new_sendler_photo_message(self, message, photo, text, keyb):
        ...
        try:
            ...
            logger_msg(f'Ошибка при отправке сообщение new_sendler_photo_message с фото {es}')
            return False
